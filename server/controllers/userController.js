...
const followOrUnfollowUserController = async (req, res) => {
    try {
        ...
        if (curUserId === userIdToFollow) {
            return res.send(error(409, "User cann't follow themselves"))
        }

        if (!userToFollow) {
            return res.send(error(404, 'User to follow no found'));
        }
        ...
        await userToFollow.save();
        await curUser.save();
        res.send(success(200, { user: userToFollow }))
